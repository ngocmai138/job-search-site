...
<body>
			...
			<div class="container">
				<div class="row">
					<div class="col-lg-12 pr-lg-5">
						<c:if test="${totalSaveJobs >0 }">
							<div class="row">
								<c:forEach var="saveJob" items="${saveJobs }">
										<div class="col-md-12 ">
											<div
												...
												<div class="one-third mb-4 mb-md-0">
													<div class="job-post-item-header align-items-center">
														...
														<h2 class="mr-3 text-black">
															<a
																href="${pageContext.request.contextPath}/recruitment/detail?recruitmentId=${saveJob.recruitment.id}">${saveJob.recruitment.title}</a>
														</h2>
													</div>
													<div class="job-post-item-body d-block d-md-flex">
														<div class="mr-3">
															<span class="icon-layers"></span> <a href="${pageContext.request.contextPath}/recruitment/showDetailCompany?companyId=${saveJob.recruitment.company.id}"
																>${saveJob.recruitment.company.nameCompany}</a>
														</div>
														<div>
															<span class="icon-my_location"></span> <span
																>${saveJob.recruitment.address}</span>
														</div>
													</div>
												</div>
												<input type="hidden"
													id="idRe${saveJob.recruitment.id}"
													value="${saveJob.recruitment.id}">
												<input type="hidden"
													id="userName${saveJob.recruitment.id}"
													value="${pageContext.request.userPrincipal.name}">
												<input type="hidden"
													id="idSJ${saveJob.recruitment.id}"
													value="${saveJob.id}">
												<div
													class="one-forth ml-auto d-flex align-items-center mt-4 md-md-0">
													<div>
														<a onclick="unfollow(${saveJob.recruitment.id})"
										...
										<div class="modal fade"
											id="exampleModal${saveJob.recruitment.id}"
															...
															<div class="row">
																...
																<div id="loai2${saveJob.recruitment.id}"
																	...
																	<label for="fileUpload" class="col-form-label">Chọn
																		...
																		id="fileUpload${saveJob.recruitment.id}"
																		name="file" required> <label for="fileUpload"
							...
							<div class="col text-center">
								<div class="block-27">
									<ul>
										...
										<li><a
											href="${pageContext.request.contextPath}/user/getListSaveJob?username=${pageContext.request.userPrincipal.name}&pageNumber=${pagePrev}&pageSize=${pageSize}">&lt;</a></li>
										</c:if>
										<c:forEach var="i" begin="1" end="${totalPages }">
											<li
												class="${pageNumber == i  ? 'active' : ' ' }"><a
												href="${pageContext.request.contextPath}/user/getListSaveJob?username=${pageContext.request.userPrincipal.name}&pageNumber=${i}&pageSize=${pageSize}"
												>${i}</a></li>
										</c:forEach>
										<c:if test="${pageNext <= totalPages}">
										<li><a
											href="${pageContext.request.contextPath}/user/getListSaveJob?username=${pageContext.request.userPrincipal.name}&pageNumber=${pageNext}&pageSize=${pageSize}">&gt;</a></li>
										</c:if>
									</ul>
								</div>
							</div>
	...
	function unfollow(id){
        ...
        $.ajax(
            {
                type: 'POST',
                url: window.location.origin + '/job-search-site/user/deleteSaveJob/',
                contentType: false,
                processData: false,
                data: formData,
                success: function (data) {
                    if(data == "false"){
                        ...
                        swal({
                            title: 'Bạn đã bỏ lưu công việc này rồi!',
                            /* text: 'Redirecting...', */
                            icon: 'error',
                            timer: 3000,
                            buttons: true,
